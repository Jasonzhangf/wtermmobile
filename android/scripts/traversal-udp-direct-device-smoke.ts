...
const relayHostId = `udp-device-${Date.now()}`;
  ...
  const expression = `(async () => {
    ...
    return await new Promise((resolve, reject) => {
      ...
      const finish = async () => {
        ...
        if (!selectedPair) throw new Error('no nominated ICE pair');
        ...
        settled = true;
        cleanup();
        resolve({ ok: true, signalTypes, selectedPair: pair, marker });
      };
      ...
      signalSocket.onmessage = async (event) => {
        try {
          ...
          if (message.type === 'rtc-answer') await peer.setRemoteDescription(new RTCSessionDescription({ type: 'answer', sdp: message.payload?.sdp || '' }));
      ...
      peer.onicecandidate = (event) => {
        if (event.candidate && signalSocket.readyState === 1) signalSocket.send(JSON.stringify({ type: 'rtc-candidate', payload: event.candidate.toJSON() }));
      };
...
async function main() {
  ...
  const tmux = spawnSync('tmux', ['new-session', '-d', '-s', tmuxSession, 'printf "udp device ready\\n"; exec bash'], { encoding: 'utf8' });
  ...
  try {
    ...
    process.stdout.write(JSON.stringify({ ok: true, relay: 'signaling-only', serial, iceServers, result, daemonLogsTail: logs.slice(-30) }, null, 2) + '\n');
  } finally {
    daemon.kill('SIGINT');
    await delay(1000);
